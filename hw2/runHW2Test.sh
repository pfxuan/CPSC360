...
if [ "$CSIP" = "130.127.201.228" ] && [ "$GRADIP" = "130.127.235.49" ]; then
  ...
  echo -e "GRADER: Test Result -> (only request is correct) Partially Passed.\n"
...
if [ $LOCALTTL -eq 0 ] && [ $CSTTL -eq 0 ]; then 
  ...
  echo -e "GRADER: Test Result -> Partially Passed.\n"
...
EBAYIPS=`./dnsq @130.127.255.250 www.ebay.com | grep IP -c & sleep 2; killall -q -9 dnsq &> /dev/null`
if [ $GOOGIPS -lt 6 ] && [ $EBAYIPS -lt 4 ] ; then
  echo -e "GRADER: Test Result -> Failed!!!\n"
  echo -e "===> Test 3 score (10 points max): 0.0 <===\n\n\n"
  P3=0
elif [ $GOOGIPS -ge 6 ] && [ $EBAYIPS -ge 4 ] ; then
  echo -e "GRADER: Test Result -> Passed.\n"
